...
export function createProposalPrompt(input: ProposalProviderInput): string {
  ...
  return JSON.stringify({
    ...
    rules: [
      ...
      'EVENT & TIMESTAMP DISAMBIGUATION (TWO DISTINCT CANONICAL EVENTS REQUIRED): When the user clarifies that an earlier reported timestamp was actually the user discovery or app-opening time, while the official event or notice occurred at an earlier timestamp: (1) Update the existing event (operations.update_event targeting EV01) to explicitly describe user discovery/app opening at the discovery time (e.g. "August 15, 2026 at 10:20 AM"); (2) Add a NEW event (operations.add_event with local_ref "new_event_1") to describe the official notice dispatch at the notice time (e.g. "August 14, 2026 at 9:47 PM"). NEVER assign the notice timestamp to the discovery event, NEVER overwrite the discovery event, and NEVER merge notice transmission and user app discovery into a single event.',
      'STRICT CLAIM ATOMICITY (NO OMNIBUS/COMPOUND FINDINGS): In both initial record turns and subsequent correction turns, create findings as distinct, atomic propositions. NEVER combine multiple distinct facts, opposing accounts, uncertainty, warning history, restriction notices, app discovery, support communications, and monetary balances into a single compound finding. Specifically: (1) Restriction notice issuance & timing (e.g. "Shopee sent the restriction notice on August 14, 2026, at 9:47 PM") is its own claim; (2) User account discovery/access failure (e.g. "The user discovered the seller account access failure upon opening the app on August 15 at 10:20 AM") is its own claim; (3) Warning notice history (e.g. "The user received a warning email regarding unusual order activity on August 12, 2026") is its own claim; (4) Support interaction/review status (e.g. "Customer support stated that the account is under review for up to seven business days") is its own claim; (5) Financial balance hold (e.g. "Approximately VND 12.4 million in seller balance is unavailable for withdrawal") is its own claim. Never merge warning history into notice timing, and never merge support review status into balance hold.',
      'CORRECTION PROVENANCE & MATCHED REASONING WARRANTS: When an intake introduces a correction (e.g. statement U02 correcting statement U01): (1) For all newly added claims (operations.add_claim) and updated claims (operations.update_claim), set source_basis_ids to the current correcting statement (e.g. ["U02"]); (2) The reasoning field for every updated or added claim MUST cite the current correcting statement (e.g. "Statement U02 clarifies...") and MUST NEVER cite prior superseded statements (e.g. U01) as the grounding basis for newly corrected propositions; (3) In operations.disposition_source, emit disposition_source operations connecting the current statement (e.g. source_id: "U02") with "supports_claim" to every claim it supports or updates, and emit relationship_type: "corrects_statement", source_id: "U02", target_ref: "U01".',
      ...
      'For reasoning.answer_status: use "recorded" (for record/correct intakes where reports are logged), "supported" (for verified findings), "conditional" (when key facts/evidence remain missing to answer the goal), or "blocked" (when an explicit blocker prevents answering). MANDATORY GAP RULE: If answer_status is "conditional" or "blocked", reasoning.steps MUST include at least one assumption step (kind: "assumption") with a non-empty gap_refs array referencing a gap (e.g. ["new_gap_1"] or ["G01"]), and that gap must exist in accepted context or be declared in operations.add_gap. If no gap is needed, answer_status MUST be "recorded" or "supported".',
